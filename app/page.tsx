...
export default function MainPage() {
  ...
  const fetchAllLocations = async () => {
    try {
      ...
      const kommunerResponse = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/kommuner`,
      );
  ...
  const filterLocations = () => {
    if (filters.nationella && !filters.ingetAvtal) {
      setFilteredLocations(
        allLocations.filter((location) => location.hasAgreement),
      );
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      const kommunResponse = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/kommuner/search?query=${query}`,
      );
    ...
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/${
          updatedLocation.isRegion ? 'counties' : 'kommuner'
        }/${updatedLocation.id}`,
        updatedLocation,
      );
    ...
    <div className="flex flex-col h-screen">
        ...
        <div className="container mx-auto">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <Button variant="medlink">Profil</Button>
            </div>
          </div>
      ...
      <main className="flex-grow flex">
        ...
        <div className="w-2/3 p-4">
          <Map
            locations={allLocations} // Pass all locations
